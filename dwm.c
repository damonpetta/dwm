...
resizeclient(Client *c, int x, int y, int w, int h)
{
    ...
    c->oldw = c->w;
    c->w = wc.width = w - (gapoffset ? (x + w + (c->bw * 2) == c->mon->wx + c->mon->ww ? 2 : 1) * gapoffset : 0);
    c->oldh = c->h;
    c->h = wc.height = h - (gapoffset ? (y + h + (c->bw * 2) == c->mon->wy + c->mon->wh ? 2 : 1) * gapoffset : 0);
